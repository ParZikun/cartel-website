...
export default function InspectPage() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            // Updated Endpoint: efficiently checks DB and falls back to live ME fetch
            const response = await fetch(`/api/inspect-card?query=${encodeURIComponent(query.trim())}`);

            if (!response.ok) {
                // Handle specific 404 or other errors
                if (response.status === 404) {
                    throw new Error('Card not found in Database or Magic Eden.');
                }
            ...
            // Hide generic 404s if it's just not found, but show real errors
            if (error.message.includes('not found')) {
                // No-op, UI handles 'card' state being null
            } else {
    ...
    return (
        <div className="w-full min-h-[calc(100vh-80px)] p-4 md:p-8 flex flex-col items-center">
            {/* Header */}
            <div className="text-center space-y-4 mb-12 mt-8">
                <h1 className="text-4xl md:text-5xl font-bold text-white font-pokemon-solid tracking-wider drop-shadow-lg">Inspect Card</h1>
                <p className="text-gray-400 text-lg max-w-xl mx-auto">Enter a Token Mint or Grading ID to view full historical details and live market data.</p>
            </div>
            ...
            <form onSubmit={handleSearch} className="w-full max-w-2xl relative group mb-8">
                ...
                <div className="relative flex items-center bg-black/60 border border-white/10 rounded-2xl p-2 focus-within:border-accent-gold/50 focus-within:bg-black/80 transition-all shadow-2xl backdrop-blur-sm">
                    ...
                    <input
                        ...
                        className="w-full bg-transparent border-none text-white text-base md:text-lg px-4 py-3 focus:outline-none placeholder:text-gray-600 font-mono"
                    />
                    <button
                        type="submit"
                        disabled={loading}
                        className="bg-accent-gold text-black font-bold px-6 py-3 rounded-xl hover:bg-accent-gold/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
                    >
            ...
            <div className="w-full max-w-6xl transition-all duration-500">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 space-y-4">
                        ...
                        <p className="text-gray-400 animate-pulse">Searching global database & live markets...</p>
                    </div>
                ) : card ? (
                    <div className="glass rounded-3xl border border-white/10 overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 bg-[#0c0a15]/80 backdrop-blur-md">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                            {/* Left: Image */}
                            <div className="lg:col-span-5 relative w-full aspect-[3/4] lg:h-auto lg:aspect-auto bg-[#05040a] border-b lg:border-b-0 lg:border-r border-white/10 group">
                                ...
                                <div className="absolute top-6 left-6 z-20">
                                    <span className={`px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-wider shadow-lg backdrop-blur-md border ${parseFloat(card.price_amount) > 0 ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-gray-500/20 text-gray-400 border-gray-500/30'}`}>
                                        {parseFloat(card.price_amount) > 0 ? 'Listed' : 'Unlisted'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-7 p-6 md:p-10 space-y-8">
                                ...
                                <div className="space-y-4">
                                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                        <h2 className="text-2xl md:text-3xl font-bold text-white leading-tight">{card.name}</h2>
                                        {card.cartel_category && card.cartel_category !== 'SKIP' && (
                                            <span className={`self-start px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-wider border whitespace-nowrap ${card.cartel_category === 'AUTOBUY' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        ...
                                        >
                                            <span className="font-mono text-sm text-accent-gold">{card.token_mint ? `${card.token_mint.slice(0, 8)}...` : 'Unknown'}</span>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-5 rounded-2xl bg-black/40 border border-white/10 relative overflow-hidden group/box hover:border-white/20 transition-colors">
                                        ...
                                        <p className="text-2xl md:text-3xl font-bold text-white">{card.price_amount ? `${Number(card.price_amount).toFixed(2)} SOL` : '-'}</p>
                                        <p className="text-sm text-gray-500 mt-1">≈ ${card.price_amount ? (card.price_amount * 150).toFixed(2) : '-'}</p>
                                    </div>

                                    <div className="p-5 rounded-2xl bg-black/40 border border-green-500/20 relative overflow-hidden group/box hover:border-green-500/30 transition-colors">
                                        <div className="absolute top-0 right-0 p-3 opacity-10 group-hover/box:opacity-20 transition-opacity">
                                        ...
                                        <p className="text-2xl md:text-3xl font-bold text-green-400">${card.alt_value ? Number(card.alt_value).toFixed(2) : '-'}</p>
                                    ...
                                    <div className="p-5 rounded-2xl bg-black/40 border border-accent-gold/20 relative overflow-hidden group/box hover:border-accent-gold/30 transition-colors">
                                        <div className="absolute top-0 right-0 p-3 opacity-10 group-hover/box:opacity-20 transition-opacity">
                                        ...
                                        <p className="text-2xl md:text-3xl font-bold text-accent-gold">${card.avg_price ? Number(card.avg_price).toFixed(2) : '-'}</p>
                                    </div>

                                    <div className="p-5 rounded-2xl bg-black/40 border border-purple-500/20 relative overflow-hidden group/box hover:border-purple-500/30 transition-colors">
                                        <div className="absolute top-0 right-0 p-3 opacity-10 group-hover/box:opacity-20 transition-opacity">
                                            <Package className="w-8 h-8 text-purple-400" />
                                        </div>
                                        <p className="text-xs text-purple-400 uppercase tracking-wider font-bold mb-1">Supply</p>
                                        <p className="text-2xl md:text-3xl font-bold text-purple-400">{card.supply || '-'}</p>
                                    </div>
                                </div>
                                ...
                                <div className="rounded-2xl border border-white/10 overflow-hidden bg-black/20">
                                    <table className="w-full text-sm">
                                        <tbody className="divide-y divide-white/5">
                                            ...
                                            <tr className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="p-4 text-right">
                                                    <span className={`px-2 py-1 rounded text-xs font-bold ${Number(card.alt_value_confidence) > 80 ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                                        {card.alt_value_confidence !== null ? `${card.alt_value_confidence}%` : 'N/A'}
                                                    </span>
                                                </td>
                                            </tr>
                                            <tr className="hover:bg-white/5 transition-colors">
                                            ...
                                            <tr className="hover:bg-white/5 transition-colors">
                                                <td className="p-4 text-gray-500 font-medium">Last Analyzed</td>
                                                <td className="p-4 text-white text-right">{card.last_analyzed_at ? new Date(card.last_analyzed_at).toLocaleString() : 'Just now'}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-300">
                        ...
                        <p className="text-gray-400 max-w-md mb-6">
                            We couldn't find a card matching that Mint Address or Grading ID in our database or live on Magic Eden.
                        </p>
                        <p className="text-sm text-yellow-500/60 bg-yellow-500/10 px-4 py-2 rounded-lg">
                            Note: Live fetching currently requires a Mint Address. Grading ID search only checks the database.
                        </p>
                    </div>
                ) : (
